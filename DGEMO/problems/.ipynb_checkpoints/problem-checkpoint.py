...
class Problem(PymooProblem):
    ...
    def evaluate(self,
        ...
        if self.elementwise_evaluation:
            ...
            out = self._evaluate_batch(X, calc_gradient, out, *args, return_values_of=return_values_of, parallel=parallel, **kwargs)
    ...
    def _evaluate_batch(self, X, calc_gradient, out, parallel, *args, **kwargs):
        # NOTE: to use self-calculated dF (gradient) rather than autograd.numpy, which is not supported by Pymoo
        self._evaluate(X, out, parallel, *args, calc_gradient=calc_gradient, **kwargs)
        at_least2d(out)
        return out
