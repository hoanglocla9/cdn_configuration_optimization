...
# def _lhscentered_int(n, intGroups):
#     """
#         initGroups: ex: [[1, 5], [7, 10]]
#     """
#     # Generate the intervals
#     ranges = [max(group) - min(group) for group in intGroups] 
#     H = np.zeros((n, len(intGroups)))
#     for idx, r in enumerate(ranges):
#         sequ = np.sort(np.random.rand(n))
#         if (len(intGroups) > 1):
#             spacing = intGroups[idx][1] - intGroups[idx][0] 
#         else:
#             raise "must have more than 1 intGroup"
#         for k in range(n): 
#             i = sequ[k] 
#             a = min(intGroups[idx]) + (i - 1)*(r + spacing)/n 
#             b = min(intGroups[idx]) + i*(r + spacing)/n - 1 
#             if (a < b):
#                 H[k][idx] = sample(range(a, b,spacing), 1) 
#             elif (a == b): 
#                 H[k][idx] = a 
#             else:
#                 raise "integer based lhs error" 
