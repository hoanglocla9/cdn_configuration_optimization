...
def build_problem(name, n_var, n_obj, n_init_sample, n_process=1, extra_params=None, mode=0):
    ...
    if name.startswith('zdt') or name == 'vlmop2':
        ...
        if name == "cdn_ram":
            ...
            topo, fileSize, colorMode, colorList, runReqNums, warmUpReqNums, separatorRankIncrement, deleteCachePath = extra_params
            problem.get_parameters(topo, fileSize, colorMode, colorList, runReqNums, warmUpReqNums, separatorRankIncrement, n_process, deleteCachePath)
        else:
            problem = get_problem(name, n_var=n_var, n_obj=n_obj)       
    # get initial samples
